...
test("Effect parser and evaluator return Effects", async () => {
  ...
  assert.equal(evaluateFilter(ast, { host: "www.example.com", status: "200" }), true);
  assert.equal(inspectAttrs({ host: "www.example.com" }), '{host: "www.example.com"}');
});
...
test("escaped asterisk is distinct from a presence filter", async () => {
  ...
  assert.equal(
    evaluateFilter(literalStar, { cn: "*" }),
    true
  );
  assert.equal(
    evaluateFilter(literalStar, { cn: "anything" }),
    false
  );

  assert.equal(
    evaluateFilter(presence, { cn: "anything" }),
    true
  );
  assert.equal(
    evaluateFilter(presence, {}),
    false
  );
