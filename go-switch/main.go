...
func parseFilter(expr string) (*node, error) {
	n, next, err := parseFilterAt(expr, 0)
	if err != nil {
		return nil, err
	}
	if next != len(expr) {
...
func parseFilterAt(expr string, pos int) (*node, int, error) {
	...
	n, err := parseFilterContent(expr[pos+1 : end])
	if err != nil {
		return nil, pos, err
	}
	return n, end + 1, nil
...
func parseFilterContent(content string) (*node, error) {
	if content == "" {
	...
	switch content[0] {
	case '&':
		children, err := parseFilterList(content[1:])
		if err != nil {
			return nil, err
		}
		if len(children) == 0 {
	...
	case '!':
		child, next, err := parseFilterAt(content, 1)
		if err != nil {
			return nil, err
		}
		if next != len(content) {
	...
	default:
		item, err := parseItem(content)
		if err != nil {
			return nil, err
		}
		return &node{kind: nodeItem, item: item}, nil
	}
}

func parseFilterList(content string) ([]*node, error) {
	var nodes []*node
	for pos := 0; pos < len(content); {
		n, next, err := parseFilterAt(content, pos)
		if err != nil {
			return nil, err
		}
...
func parseItem(content string) (itemNode, error) {
	opIndex := -1
	op := byte(0)
	...
	if opIndex < 0 {
		return itemNode{}, parseError{message: "error in item syntax"}
	}
	...
	return itemNode{attr: attr, op: opText, value: value, wildcard: wildcard}, nil
}
...
func (n *node) match(attrs orderedAttrs) bool {
	switch n.kind {
	case nodeItem:
		return n.item.match(attrs)
	case nodeAnd:
	...
	case nodeOr:
		...
		return false
	case nodeNot:
		return !n.child.match(attrs)
	default:
		return false
	}
}

func (n itemNode) match(attrs orderedAttrs) bool {
...
func processInput(reader *bufio.Reader, format formatKind, ast *node, stdout io.Writer) error {
	switch format {
	case formatCSV:
...
func processCSV(reader *bufio.Reader, ast *node, stdout io.Writer) error {
...
func processLTSV(reader *bufio.Reader, ast *node, stdout io.Writer) error {
	for {
